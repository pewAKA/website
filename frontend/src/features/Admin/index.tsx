  ...
  return (
    <main className="admin-shell">
      <header className="admin-shell__header">
        <Link className="admin-shell__brand" href="/">
  ...
  return (
    <section className="admin-page">
      <header className="admin-page__heading">
        ...
        <Button type="primary" onClick={() => router.push('/admin/articles/new')}>
          新建文章
        </Button>
      </header>
          ...
          {result.items.map((article) => (
            <article key={article.id}>
              ...
              <div className="admin-article-list__actions">
                <Button onClick={() => router.push(`/admin/articles/${article.id}`)}>编辑</Button>
                <Button
                  loading={
...
export function AdminArticleEditor() {
  ...
  return (
    <section className="admin-page admin-editor">
      <header className="admin-page__heading">
        <div>
          <Link href="/admin/articles">← 返回文章库</Link>
      ...
      {categories.length === 0 && (
        <p className="admin-editor__notice">
          请先在 <Link href="/admin/taxonomy">分类与标签</Link> 中创建至少一个启用的分类。
        </p>
      )}
      <Form<EditorValues>
